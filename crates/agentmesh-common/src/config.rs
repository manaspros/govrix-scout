...
//!
//! All environment variables use the `AGENTMESH_` prefix.
...
pub struct DatabaseConfig {
    /// PostgreSQL connection URL. Use `AGENTMESH_DATABASE_URL` env var in production.
    pub url: String,
...
impl Default for DatabaseConfig {
    fn default() -> Self {
        Self {
            url: "postgres://agentmesh:agentmesh@localhost:5432/agentmesh".to_string(),
            max_connections: 20,
            min_connections: 2,
...
impl Config {
    ...
    ///
    /// Environment variables (all prefixed with `AGENTMESH_`):
    /// - `AGENTMESH_DATABASE_URL`        → database.url
    /// - `AGENTMESH_PROXY_PORT`          → proxy.port
    /// - `AGENTMESH_API_PORT`            → api.port
    /// - `AGENTMESH_LOG_LEVEL`           → telemetry.log_level
    /// - `AGENTMESH_PROXY_FAIL_OPEN`     → proxy.fail_open
    ...
    /// Apply `AGENTMESH_*` environment variable overrides.
    fn apply_env_overrides(&mut self) {
        if let Ok(url) = std::env::var("AGENTMESH_DATABASE_URL") {
            self.database.url = url;
        }
        if let Ok(port) = std::env::var("AGENTMESH_PROXY_PORT") {
            if let Ok(p) = port.parse() {
                self.proxy.port = p;
            }
        }
        if let Ok(port) = std::env::var("AGENTMESH_API_PORT") {
            if let Ok(p) = port.parse() {
                self.api.port = p;
            }
        }
        if let Ok(level) = std::env::var("AGENTMESH_LOG_LEVEL") {
            self.telemetry.log_level = level;
        }
        if let Ok(val) = std::env::var("AGENTMESH_PROXY_FAIL_OPEN") {
