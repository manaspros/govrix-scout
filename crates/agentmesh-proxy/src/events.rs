//! Event channel and background writer for AgentMesh proxy.
//!
//! Architecture:
...
use agentmesh_common::models::agent::{Agent, AgentType};
...
pub async fn run_background_writer(
    ...
    pool: Option<agentmesh_store::StorePool>,
    metrics: Arc<Metrics>,
) {
...
async fn flush_batch(
    ...
    match pool {
        Some(p) => {
            match agentmesh_store::events::insert_events_batch(p, batch).await {
                Ok(count) => {
...
async fn upsert_agents_from_batch(
    ...
    for (agent_id, accum) in &agents {
        ...
        if let Err(e) = agentmesh_store::agents::upsert_agent(pool, &agent).await {
            tracing::warn!(
                error = %e,
                agent_id = %agent_id,
...
mod tests {
    ...
    fn event_channel_try_send_non_blocking() {
        ...
        rt.block_on(async {
            ...
            let event = agentmesh_common::models::event::AgentEvent::new(
                "agent-1",
                uuid::Uuid::now_v7(),
                agentmesh_common::models::event::EventDirection::Outbound,
                "POST",
                "https://api.openai.com/v1/chat/completions",
                agentmesh_common::models::event::Provider::OpenAI,
                "genesis",
                "audit:none",
            );
