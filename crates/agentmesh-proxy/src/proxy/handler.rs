...
fn refine_streaming_from_body(protocol: Protocol, body: &Bytes) -> Protocol {
    match protocol {
        Protocol::OpenAI { version, .. } => {
            let streaming = agentmesh_common::protocols::openai::is_streaming(body);
            Protocol::OpenAI { version, streaming }
        }
        Protocol::Anthropic { version, .. } => {
            let streaming = agentmesh_common::protocols::anthropic::is_streaming(body);
...
fn detect_mcp_transport(headers: &http::HeaderMap) -> agentmesh_common::protocols::McpTransport {
    if headers
        ...
        .unwrap_or(false)
    {
        agentmesh_common::protocols::McpTransport::Sse
    } else {
        agentmesh_common::protocols::McpTransport::JsonRpc
    }
}
