...
pub async fn log_request_event(ctx: &RequestContext, state: &InterceptorState) {
    ...
    match &ctx.protocol {
        ...
        Protocol::Anthropic { .. } => {
            if let Some(req) =
                agentmesh_common::protocols::anthropic::parse_request(&ctx.request_body)
            {
                event.model = Some(req.model);
            }
        }
...
pub async fn log_response_event(
    ...
    match &ctx.protocol {
        Protocol::OpenAI { streaming, .. } => {
            if *streaming {
                ...
                let acc = agentmesh_common::protocols::openai::parse_streaming_chunks(&data_lines);
                event.model = acc.model.or_else(|| {
                    agentmesh_common::protocols::openai::parse_model(&ctx.request_body)
                });
                ...
                agentmesh_common::protocols::openai::parse_response(response_body)
            {
        ...
        Protocol::Anthropic { streaming, .. } => {
            if *streaming {
                ...
                agentmesh_common::protocols::anthropic::parse_response(response_body)
            {
                event.model = Some(resp.model);
    ...
    if let (Some(input), Some(output)) = (event.input_tokens, event.output_tokens) {
        if let Some(ref model_name) = event.model {
            if let Some(pricing) = agentmesh_common::models::pricing::lookup_pricing(model_name) {
...
pub async fn analyze_request(
    ...
    match protocol {
        Protocol::OpenAI { .. } => {
            let model = agentmesh_common::protocols::openai::parse_model(body);
            tracing::debug!(
                agent = agent_id,
                model = ?model,
                bytes = body.len(),
        ...
        Protocol::Anthropic { .. } => {
            let model = agentmesh_common::protocols::anthropic::parse_model(body);
            tracing::debug!(
                agent = agent_id,
                model = ?model,
                bytes = body.len(),
        ...
        Protocol::Mcp { server, .. } => {
            if let Some(req) = agentmesh_common::protocols::mcp::parse_request(body) {
                let tool = agentmesh_common::protocols::mcp::extract_tool_name(&req);
                tracing::debug!(
                    agent = agent_id,
                    server = server,
...
pub async fn analyze_response(
    ...
    match protocol {
        Protocol::OpenAI { .. } => {
            let _usage = agentmesh_common::protocols::openai::parse_usage(body);
        }
        Protocol::Anthropic { .. } => {
            let _usage = agentmesh_common::protocols::anthropic::parse_usage(body);
        }
        _ => {}
    }
